...
import { ShieldAlert, ChevronLeft, Activity, Wallet } from 'lucide-react';
    ...
    useEffect(() => {
        const fetchSchemes = async () => {
            try {
                ...
                const res  = await fetch(`${API_URL}/api/schemes?crop=${user?.crop_type || user?.crop || ''}&location=${user?.location || ''}`);
                const data = await res.json();
                setSchemes(data);
            } catch {
                setSchemes([
                    { id: 1, title: 'Pradhan Mantri Fasal Bima Yojana (PMFBY)', description: 'Crop insurance scheme providing financial support in case of crop failure.', tags: ['Insurance', user?.crop_type || 'Cotton'], type: 'green', link: 'https://pmfby.gov.in/' },
                    ...
                    { id: 4, title: 'Soil Health Card Scheme',                    description: 'Helps farmers understand soil health and improve crop yield.',             tags: ['Soil', 'Sustainability'],                    type: 'green', link: 'https://soilhealth.dac.gov.in/' },
                    ...
                    { id: 6, title: 'Paramparagat Krishi Vikas Yojana',           description: 'Promotes organic farming to improve soil health and sustainability.',      tags: ['Organic', 'Eco-friendly'],                   type: 'orange',link: 'https://pgsindia-ncof.gov.in/pkvy/index.aspx' },
                ]);
            } finally {
                setLoading(false);
            }
    ...
    return (
        <div className="min-h-screen bg-transparent flex flex-col font-sans">
            ...
            <header className="sticky top-0 z-50 bg-[#121212]/90 backdrop-blur-md border-b border-gray-800 p-4 flex items-center shadow-md">
                <button onClick={() => navigate(-1)} className="p-2 mr-3 bg-gray-800/50 rounded-full text-gray-300 hover:text-white hover:bg-gray-700 transition">
                    <ChevronLeft size={20} />
                </button>
                <div className="flex-1">
                    <h1 className="text-xl font-bold text-white">Recommended Schemes</h1>
                    <p className="text-xs text-gray-400">
                        Tailored for {user?.crop_type || user?.crop || 'your farm'} in {user?.location ? user.location.split(',')[0] : 'your area'}
                    </p>
                </div>
            </header>
            ...
            <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4">
                {loading ? (
                    Array(5).fill(0).map((_, i) => (
                        <div key={i} className="animate-pulse bg-[#121418] rounded-2xl border border-gray-800 h-28 w-full" />
                    ))
                ) : schemes.length > 0 ? (
                    schemes.map((scheme, idx) => {
                        ...
                        return (
                            <div
                                key={scheme.id || idx}
                                className={`group bg-[#121418]/80 backdrop-blur-md rounded-2xl border border-gray-800/60 p-5 ${theme.border} border-l-[4px] flex flex-col md:flex-row md:items-center justify-between gap-5 transition-all hover:bg-[#1a1d24]`}
                            >
                                <div className="flex items-start">
                                    ...
                                    <div>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {scheme.tags?.map(t => (
                                                <span key={t} className="text-xs bg-gray-800/80 text-gray-300 px-3 py-1 rounded-full border border-gray-700 font-medium">
                                                    {t}
                                                </span>
                                            ))}
                                        </div>
